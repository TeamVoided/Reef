...
not change vanilla but adding to it. So lets say we want to be able to customize the Ice Spike feature, and the way Reef
...
While this lib is only in the early stages of development. We arely have plans to Fully deprecate this mod. Since This
is planed to be just one module form out future mod `VoidLib`.

### Why?

Because it more nice to have a single jar file for all the libs we use, kinda like Fapi or QSL
...
All the current features where made quickly and very messily for a world gen mod we were working on. They have not been
property cleaned up and optimised. So do expect them to change, both as features and their configs.
The future vision for this mod is that the Features that are modified version of vanilla ones. Act exactly like vanilla
ones but have the ability to be configured (And optimised where possible).

## Wiki

As of the current update (0.1.2) there is no wiki.
This is just a note saying that Ender is working on one.
If there isn't one more then a month after the (0.1.2) release go @ Ender on discord
